...
namespace Gini\ORM\Order\Review;
...
class Request extends \Gini\ORM\Object
{
    ...
    //  通过的操作状态应该是奇数
    //  拒绝的操作状态应该是偶数
    const STATUS_SCHOOL_PASSED = 1; // 学院管理方审核通过
    const STATUS_SCHOOL_FAILED = 2; // 学院管理方审核拒绝
    const STATUS_UNIVERS_PASSED = 3; // 学校管理方审核通过
    const STATUS_UNIVERS_FAILED = 4; // 学校管理方审核拒绝

    public static $status_titles = [
        self::STATUS_PENDING=> '待处理',
        self::STATUS_SCHOOL_PASSED=> '院系审核通过',
        self::STATUS_SCHOOL_FAILED=> '院系已拒绝',
    ...
    public static function getAllowedPendingStatus($user, $group)
    {
        ...
        foreach ($actions as $action) {
            if ($action->type==\Gini\ORM\Hazardous\Review\Action::TYPE_STEP_SCHOOL) {
                $result[$action->code][] = self::STATUS_PENDING;
            }
            else if ($action->type==\Gini\ORM\Hazardous\Review\Action::TYPE_STEP_UNIVERS) {
                $result[$action->code][] = self::STATUS_SCHOOL_PASSED;
            }
        }
    ...
    public static function getAllowedDoneStatus($user, $group)
    {
        ...
        foreach ($actions as $action) {
            if ($action->type==\Gini\ORM\Hazardous\Review\Action::TYPE_STEP_SCHOOL) {
                $result[$action->code][] = self::STATUS_SCHOOL_PASSED;
                $result[$action->code][] = self::STATUS_SCHOOL_FAILED;

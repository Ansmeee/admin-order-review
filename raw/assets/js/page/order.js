define('page/order', ['jquery', 'utils/bootbox', 'board'], function(jQuery, Bootbox, Board) {
    $(document).on('click', '.app-op-per-handler', function() {
        var url = 'ajax/order/review/get-op-form';
    ...
    $(document).on('click', '.app-pager-li-handler', function() {
        ...
        var url = ['ajax/order/review/more', page, type].join('/');
        search({
            url: url
            ,q: q
        });
